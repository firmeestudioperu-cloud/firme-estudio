            ...
            <div className="inline-flex items-center gap-1.5">
              ...
              >
                {currentUser ? (
                  <>
                    {currentUser.avatar ? (
                      <img src={currentUser.avatar} alt={currentUser.name || 'Alumna'} className="w-4 h-4 rounded-full object-cover" />
                    ) : (
                      <UserCheck className="w-3.5 h-3.5 text-[#B5654A]" />
                    )}
                    <span className="font-semibold max-w-[85px] truncate">{(currentUser.name || 'Alumna').split(' ')[0]}</span>
                    {isStaff ? (
                      <span
          ...
          >
            ...
            <div className="pt-2 px-3 space-y-2">
              ...
              >
                ...
                <span>{currentUser ? `Mi Cuenta (${(currentUser.name || 'Alumna').split(' ')[0]})` : 'Check-in / Ingresar con Gmail'}</span>
              </button>
